...
export default function App() {
  ...
  // Persist tournaments to localStorage whenever they change
  useEffect(() => { saveTournaments(tournaments); }, [tournaments]);
  useEffect(() => { savePlayers(players); }, [players]);
  useEffect(() => { saveRankings(rankings); }, [rankings]);
          ...
          <div>
            ...
            {rounds.map((round, ri) => (
              <Card key={ri} style={{ marginBottom: 12 }}>
                <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: colors.gray600 }}>{T("round")} {ri + 1}</h4>
                {round.map((m) => (
